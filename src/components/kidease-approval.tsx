  ...
  return (
    <aside className="ke-trust" data-ke="kidease-approval" data-ke-approval-audience={audience}>
      <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0">
        ...
        <p className="font-display text-[0.95rem] leading-5 tracking-tight text-fg">{t("kideaseApprovedTitle")}</p>
      </div>
      ...
      <ul className="mt-1.5 flex flex-wrap gap-1">
        {checks.map((label) => (
          <li
            key={label}
            className="rounded-full bg-surface/90 px-2 py-0.5 text-[11px] font-medium leading-4 text-fg ring-1 ring-primary/20"
          >
            {label}
          </li>
        ))}
