...
export function CityHubLinks({ className }: { className?: string }) {
  ...
  return (
    <nav aria-label={t("browseCities")} className={className}>
      ...
      <ul className="mt-2 flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] lg:flex-wrap lg:overflow-visible [&::-webkit-scrollbar]:hidden">
        {hubs.map((hub) => {
