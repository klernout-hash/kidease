...
export function AdminLicenseActions({
  ...
  return (
    <div className="mt-3 space-y-2">
      {showSignals ? <TrustSignals item={item as TrustListing} surface="admin" compact /> : null}
      ...
      <div className="flex flex-wrap gap-2">
        <button type="button" disabled={busy} onClick={() => onReview("matched")} className="inline-flex min-h-11 items-center rounded-full bg-ok/15 px-3 text-xs text-ok disabled:opacity-50">
          Mark registry-matched
        </button>
        <button type="button" disabled={busy} onClick={() => onReview("mismatch")} className="inline-flex min-h-11 items-center rounded-full bg-surface-2 px-3 text-xs disabled:opacity-50">
          Mark mismatch
        </button>
        <button type="button" disabled={busy} onClick={() => onReview("expired")} className="inline-flex min-h-11 items-center rounded-full bg-danger/10 px-3 text-xs text-danger disabled:opacity-50">
          Mark expired
        </button>
        <button type="button" disabled={busy} onClick={() => onReview("suspended")} className="inline-flex min-h-11 items-center rounded-full bg-danger/10 px-3 text-xs text-danger disabled:opacity-50">
          Mark suspended
        </button>
        <button type="button" disabled={busy} onClick={() => onReview("unverified")} className="inline-flex min-h-11 items-center rounded-full bg-surface-2 px-3 text-xs text-muted disabled:opacity-50">
          Clear to unverified
        </button>
      </div>
    </div>
